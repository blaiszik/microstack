    ...
    try:
        ...
        if structure_dir and structure_dir.exists():
            ...
            if final_state.relaxation_results and final_state.structure_info:
                from microstack.relaxation.relax_report_generator import generate_natural_description
                formula = final_state.structure_info.get("formula", "Unknown")
                analysis = {
                    "energy_change_eV": final_state.relaxation_results.get("energy_change", 0),
                    "relaxation": {
                        "n_atoms": final_state.structure_info.get("num_atoms", 0),
                    },
                    "microscopy_results": final_state.microscopy_results or {},
                }
                ai_summary_md = generate_natural_description(formula, "surface", analysis)
            ...
            for asset_path in structure_dir.rglob("*"):
                if asset_path.suffix.lower() in [".png", ".jpg", ".jpeg", ".gif", ".xyz"]:
...
if OUTPUT_DIR.exists():
    app.mount("/output", StaticFiles(directory=str(OUTPUT_DIR.absolute())), name="output")

@app.get("/")
async def root():
    return {"message": "µ-Stack API is running"}
