...
def try_read_with_pynsid(filepath: Path, verbose: bool = True) -> list:
    ...
    """
    if verbose:
        print(f"\n🔍 Reading with pyNSID.io.hdf_utils.read_h5py_dataset()...")
        ...
        for key in h5_obj.keys():
            ...
            if isinstance(obj, h5py.Dataset):
                ...
                try:
                    ...
                    if verbose:
                        print(f"   ✅ {full_path}: Read successfully")
...
def analyze_nsid_file(filepath: Path, output_dir: Path = None, verbose: bool = True):
    ...
    print(f"\n🔬 Analyzing NSID file with pycroscopy...")
    print("=" * 60)
    
    if PYCROSCOPY_AVAILABLE:
        print(f"   Using pycroscopy v{px.__version__}")
    else:
        print("   ⚠️  pycroscopy not available, using basic analysis")
...
def analyze_image_with_pycroscopy(dataset, data, filepath, path, output_dir, verbose):
    ...
    if PYCROSCOPY_AVAILABLE:
        try:
            print(f"\n   🔬 FFT Power Spectrum (pycroscopy.fft):")
...
def analyze_spectral_image_with_pycroscopy(dataset, data, filepath, path, output_dir, verbose):
    ...
    print(f"\n   🎨 Spectral Image Analysis (pycroscopy):")
    print(f"      Original shape: {data.shape}")
    
    # Matrix Factorization using pycroscopy
    if PYCROSCOPY_AVAILABLE:
        try:
            print(f"\n   🔬 Matrix Factorization (pycroscopy.learn.MatrixFactor):")
            ...
            if np.all(data >= 0):
                ...
                print(f"      ✅ NMF decomposition successful")
            ...
            print(f"      ⚠️  Matrix factorization failed: {e}")
    ...
    plt.savefig(fig_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n   💾 Saved spectral analysis figure: {fig_path}")
...
def analyze_spectrum(dataset, data, filepath, path, output_dir, verbose):
    ...
    try:
        ...
        peaks = []
        x_vals = np.arange(len(data))
        print("      ⚠️  scipy not available for peak finding")
    ...
    plt.savefig(fig_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n   💾 Saved spectrum figure: {fig_path}")
...
def main():
    ...
    print(f"\n🔬 MicroStack NSID Validator & Analyzer")
    ...
    print(f"\n{'='*60}")
    if all_valid:
        print(f"✅ All {len(files)} file(s) are valid NSID format")
    else:
        print(f"⚠️  Some files have issues")
