...
def run_afm_simulation(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        # Extract parameters
        pix_per_angstrome = get_param("afm_pix_per_angstrome", settings.afm_pix_per_angstrome)
        ...
        logger.info(f"AFM parameters: pixPerAngstrome={pix_per_angstrome}, scan_dim={scan_dim}, iZPP={i_zpp}")
        ...
        if not PPAFM_AVAILABLE:
            logger.warning("ppafm with OpenCL support not available. Saving AFM configuration without running simulation.")
            ...
            afm_config = {
                ...
                "parameters": {
                    ...
                    "df_steps": df_steps,
                    "tip_r0": tip_r0 if isinstance(tip_r0, (list, tuple)) else list(tip_r0),
                    "tip_stiffness": tip_stiffness if isinstance(tip_stiffness, (list, tuple)) else list(tip_stiffness),
                    ...
                    "scan_size": scan_size if isinstance(scan_size, (list, tuple)) else list(scan_size),
                }
            }
            ...
            state.add_error("AFM simulation skipped: ppafm/OpenCL not available. Install OpenCL runtime and ppafm[opencl] to enable AFM.")
            return state
        ...
        try:
            ...
            atomic_disks = AtomicDisks(scan_dim=scan_dim_2d, scan_window=scan_window_2d, zmin=-1.2)
            height_map = HeightMap(scanner=afmulator.scanner, zmin=-2.0)
            es_map = ESMapConstant(
                scan_dim=scan_dim_2d, scan_window=scan_window_2d, vdW_cutoff=-2.0, Rpp=1.0
            )
            ...
            if "BUILD_PROGRAM_FAILURE" in str(e) or "clBuildProgram" in str(e):
                ...
                state.file_paths["microscopy"] = str(afm_dir)
                state.add_warning("AFM simulation skipped: OpenCL compilation failed. Try updating pocl or ppafm packages.")
                return state
            else:
